...
# The script contains the loops to cycle through all of the completed simulation files.
# External verification scripts are called from within the loops in this script.

# Path to base simulation directory
pathBaseSimulation="/home/ukurien/projects/def-yaumanko/ukurien/completed_runs"

# Path to base log directory
pathBaseLog="/home/ukurien/projects/def-yaumanko/ukurien/completed_runs/Logs"
...
tempLogNo=0
tempLogNo=$(ls -l $pathBaseLog/tempLogColBulk_Completed_No*.log | wc -l)
echo "Meoooooooooooooooooooooooow: $tempLogNo"
nameLogFile=tempLogColBulk_Completed_No$tempLogNo.log
...
# Path to subscripts
pathSubScripts="/home/ukurien/projects/def-yaumanko/ukurien/Github/Scripts/verification_scripts_for_collision_efficiency_master_bidisperse_COMPLETED_RUNS/"
...
printf "%-5s %-8s %-8s %-8s %-7s %-9s %-13s %-7s %-8s %-7s %-10s %-6s\n" "SNo" "R" "r" "TND" "EDR" "nstop" "Wall" "Gomic" "iHydro" "Col." "Run time" "CE" >> $pathLogFile
	...
	do
		...
		do
			...
			then
				...
				# Assigning the number of collisions of gomic2ihydro0 to a var for calc of CE
				colNumber_gomic2ihydro0=$colNumber
			...
			then
				...
				# Assigning the number of collisions of gomic2ihydro0 to a var for calc of CE
				colNumber_gomic2ihydro1=$colNumber
				
				# Shell instructions can be included directly beneath this
				# Calculating CE
				colEfficiency=$(echo "scale=3; ($colNumber_gomic2ihydro1/$colNumber_gomic2ihydro0)*100" | bc)
				
				# Recording data to file
				printf "%-5s %-8s %-8s %-8s %-7s %-9s %-13s %-7s %-8s %-7s %-10s %-6s\n" ""$counter"b." "$dropSizeR_ret" "$dropSizer_ret" "$tnd_ret" "$edr_ret" "$nstop_ret" "$wall_ret" "$gomic_ret" "$ihydro_ret" "$colNumber" "$runTime" "$colEfficiency" >> $pathLogFile
