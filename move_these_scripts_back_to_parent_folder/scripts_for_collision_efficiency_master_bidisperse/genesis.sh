...
lbrbyR=0.2 #lower bound ratio of collected to collector drop
...
#			colDropSize=$(echo "scale=0;$colDropSize_half*2" | bc)
			...
			echo "$counter_sno_logfile. EDR=$EDR, DS=$domainSize,r/R=$rbyR, R=$DropSize, r=$colDropSize, ND=$numberOfDrops, TND=$numberOfDrops_Total, RLWC=$reCalcLWC; NP=$numberOfProcs" >> $pathLogFiles/edr_dropsize_dropnumber.log
...
echo "/------------- Droplet number and LWC statistics-------------------------------------/" >> $pathLogFiles/edr_dropsize_dropnumber.log
echo "Average number of drops: $numberOfDrops_avg" >> $pathLogFiles/edr_dropsize_dropnumber.log
echo "Standard deviation of number of drops: $numberOfDrops_std" >> $pathLogFiles/edr_dropsize_dropnumber.log
echo "Average LWC: $reCalcLWC_avg" >> $pathLogFiles/edr_dropsize_dropnumber.log
echo "Standard deviation of LWC: $reCalcLWC_std" >> $pathLogFiles/edr_dropsize_dropnumber.log

#------------------------ D E B U G G I N G    A N D    D I A G N O S T I C S ---------------------#
echo "summation all drops=$numberOfDrops_sum average drop no=$numberOfDrops_avg summation LWC=$reCalcLWC_sum avg LWC=$reCalcLWC_avg counter=$counter_sno_logfile reCalcLWC numerator=$reCalcLWC_std_numerator" >> $pathLogFiles/edr_dropsize_dropnumber.log
echo "scale=10;((${reCalcLWC_array1[$i]}-$reCalcLWC_avg)^2)+0" | bc >> $pathLogFiles/edr_dropsize_dropnumber.log
